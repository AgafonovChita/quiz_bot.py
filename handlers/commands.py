...
async def select_topic(callback: CallbackQuery, bot: Bot, db_engine: DB_engine):
    ...
    await callback.message.edit_text("Select QUIZ", reply_markup=type_quiz_keyboard)


# check topic
async def check_topic(callback: CallbackQuery, callback_data: DataTopic, db_engine: DB_engine):
    await callback.answer()
    topic_data = await db_engine.get_topic_info(callback_data.topic_id)
    await callback.message.answer(text='ID topic: ' + topic_data[0] + '\n' +
                                       'Topic: ' + topic_data[1] + '\n' +
                                       'Описание: ' + topic_data[2] + '\n' +
                                       'Автор: ' + topic_data[3] + '\n' +
                                       'Кол-во вопросов: ' + str(topic_data[4]) + '\n' +
                                       'Время на ответ: ' + str(topic_data[5] + ' секунд \n'),
                                  reply_markup=await generate_goquiz_key())


# register command - main Router
def register_commands_main(router: Router):
    router.callback_query.register(select_topic, F.data == "start_quiz")
